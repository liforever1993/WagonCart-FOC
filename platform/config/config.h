...
#define BAT_DEAD                (320 * BAT_CELLS * BAT_CALIB_ADC) / BAT_CALIB_REAL_VOLTAGE    // All leds off: undervoltage poweroff. (while not driving) [V*100/cell]. In this case 3.20 V/cell
// ######################## END OF BATTERY ###############################
...
// Hall mapping option
// #define HALL_MAP_60_DEG               // Uncomment for common 60 deg hall placement.
                                        // If rotation direction is wrong, swap any 2 motor phases or adjust hall wiring.
...
#define N_MOT_MAX       160             // [rpm] Maximum motor speed limit; with 6.5-inch wheels this is about 5.0 km/h no-load (varies with battery/load)
...
#define DEFAULT_RATE                64   // 4.0f [-] lower value == slower rate [0, 32767] = [0.0, 2047.9375]. Do NOT make rate negative (>32767)
#define DEFAULT_FILTER              6553  // Default for FILTER 0.1f [-] lower value == softer filter [0, 65535] = [0.0 - 1.0].
#define DEFAULT_SPEED_COEFFICIENT   8192 // Default for SPEED_COEFFICIENT 0.5f [-] higher value == stronger. [0, 65535] = [-2.0 - 2.0]. In this case 8192 = 0.5 * 2^14
#define DEFAULT_STEER_COEFFICIENT   4096  // Defualt for STEER_COEFFICIENT 0.25f [-] higher value == stronger. [0, 65535] = [-2.0 - 2.0]. In this case  4096 = 0.25 * 2^14. If you do not want any steering, set it to 0.
