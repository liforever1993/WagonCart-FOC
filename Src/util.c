...
// Timeout Flag for ADC Protection: 0 = OK, 1 = Problem detected (line disconnected or wrong ADC data)
uint8_t timeoutFlgADC = 0;
// Timeout Flag for Rx Serial command: 0 = OK, 1 = Problem detected (line disconnected or wrong Rx data)
...
uint16_t VirtAddVarTab[NB_OF_VAR] = {1000, 1001, 1002, 1003, 1004, 1005, 1006, 1007, 1008, 1009, 1010,
...
// Timeout counter for Rx Serial command
static uint16_t timeoutCntSerial_L = SERIAL_TIMEOUT;
// Timeout Flag for Rx Serial command: 0 = OK, 1 = Problem detected (line disconnected or wrong Rx data)
static uint8_t timeoutFlgSerial_L = 0;
#endif
...
// Timeout counter for Rx Serial command
static uint16_t timeoutCntSerial_R = SERIAL_TIMEOUT;
// Timeout Flag for Rx Serial command: 0 = OK, 1 = Problem detected (line disconnected or wrong Rx data)
...
{
    return (map[0] < 3 && map[1] < 3 && map[2] < 3 && map[0] != map[1] && map[0] != map[2] && map[1] != map[2]);
}
...
void BLDC_Init(void)
{
    ...
    rtP_Left.b_angleMeasEna =
        0; // Motor angle input: 0 = estimated angle, 1 = measured angle (e.g. if encoder is available)
...
void Input_Init(void)
{
    ...
    if (writeCheck == FLASH_WRITE_KEY) {
        ...
        for (uint8_t i = 0; i < INPUTS_NR; i++) {
            if (input1[i].typDef ==
                3) { // If Input type defined is 3 (auto), identify the input type based on the values from config.h
...
void UART_DisableRxErrors(UART_HandleTypeDef *huart)
{
    ...
    CLEAR_BIT(huart->Instance->CR3,
              USART_CR3_EIE); /* Disable EIE (Frame error, noise error, overrun error) interrupts */
}
#endif
 ...
 * The Values will be saved to flash. Values are persistent if you flash with platformio. To erase them, make a full
 * chip erase.
 */
void adcCalibLim(void)
{
    ...
    while (!HAL_GPIO_ReadPin(BUTTON_PORT, BUTTON_PIN) && input_cal_timeout++ < 4000) { // 20 sec timeout
        ...
        INPUT1_MID_temp =
            (int16_t)(input1_fixdt >>
                      16); // CLAMP(input1_fixdt >> 16, INPUT1_MIN, INPUT1_MAX);   // convert fixed-point to integer
...
void updateCurSpdLim(void)
{
    ...
    cur_factor = CLAMP((input1_fixdt - (input1[inIdx].min << 16)) / (input1[inIdx].max - input1[inIdx].min), 6553,
                       65535); // ADC1, MIN_cur(10%) = 1.5 A
    spd_factor = CLAMP((input2_fixdt - (input2[inIdx].min << 16)) / (input2[inIdx].max - input2[inIdx].min), 3276,
                       65535); // ADC2, MIN_spd(5%)  = 50 rpm

    if (input1[inIdx].typ != 0) {
        // Update current limit
        rtP_Left.i_max = rtP_Right.i_max =
            (int16_t)((I_MOT_MAX * A2BIT_CONV * cur_factor) >> 12); // fixdt(0,16,16) to fixdt(1,16,4)
...
void standstillHold(void)
{
    ...
    if (!rtP_Left.b_cruiseCtrlEna) { // If Stanstill in NOT Active -> try Activation
        if (((input1[inIdx].cmd > 50 || input2[inIdx].cmd < -50) &&
             speedAvgAbs < 30)                                // Check if Brake is pressed AND measured speed is small
        ...
        if (input1[inIdx].cmd < 20 && input2[inIdx].cmd > 50 &&
            !cruiseCtrlAcv) { // Check if Brake is released AND Throttle is pressed AND no Cruise Control
...
#else
    if (!rtP_Left.b_cruiseCtrlEna) { // If Stanstill in NOT Active -> try Activation
        // If Speed is less than 5, and both steering and speed inputs are very small (ABS(input2.cmd) < 20 &&
        // ABS(input1.cmd) < 20), we can consider the vehicle to be at a standstill, and the user is not trying to
        ...
        // Check the steering input (input1) and speed input (input2) to determine if the user is trying to control the
        // vehicle. If either of these inputs exceeds a certain threshold (ABS(input2.cmd) > 50 || ABS(input1.cmd) >
        // 50), and no cruise control is active, we can assume that the user wants to exit the standstill hold state, so
        // we should deactivate cruise control to allow the user to control the vehicle.
        if (((ABS(input2[inIdx].cmd) > 50) || (ABS(input1[inIdx].cmd) > 50)) && !cruiseCtrlAcv) {
            rtP_Left.b_cruiseCtrlEna  = 0;
            rtP_Right.b_cruiseCtrlEna = 0;
            standstillAcv             = 0;
        }
 ...
 * In case of TORQUE mode, this function replaces the motor "freewheel" with a constant braking when the input torque
 * request is 0. This is useful when a small amount of motor braking is desired instead of "freewheel".
 *
...
void electricBrake(uint16_t speedBlend, uint8_t reverseDir)
{
    ...
    if (input2[inIdx].cmd >= 0 && input2[inIdx].cmd < ELECTRIC_BRAKE_THRES) {
        input2[inIdx].cmd =
            MAX(brakeVal, ((ELECTRIC_BRAKE_THRES - input2[inIdx].cmd) * brakeVal) / ELECTRIC_BRAKE_THRES);
    }
    else if (input2[inIdx].cmd >= -ELECTRIC_BRAKE_THRES && input2[inIdx].cmd < 0) {
        input2[inIdx].cmd =
            MIN(brakeVal, ((ELECTRIC_BRAKE_THRES + input2[inIdx].cmd) * brakeVal) / ELECTRIC_BRAKE_THRES);
    }
    else if (input2[inIdx].cmd >= ELECTRIC_BRAKE_THRES) {
        input2[inIdx].cmd = MAX(brakeVal, ((input2[inIdx].cmd - ELECTRIC_BRAKE_THRES) * INPUT_MAX) /
                                              (INPUT_MAX - ELECTRIC_BRAKE_THRES));
    }
    else { // when (input2.cmd < -ELECTRIC_BRAKE_THRES)
        input2[inIdx].cmd = MIN(brakeVal, ((input2[inIdx].cmd + ELECTRIC_BRAKE_THRES) * INPUT_MIN) /
...
void cruiseControl(uint8_t button)
{
    ...
    else if (button && rtP_Left.b_cruiseCtrlEna &&
             !standstillAcv) { // Cruise control deactivated if no Standstill Hold is active
        rtP_Left.b_cruiseCtrlEna  = 0;
        rtP_Right.b_cruiseCtrlEna = 0;
        cruiseCtrlAcv             = 0;
        beepShortMany(2, -1);
...
void readInputRaw(void)
{
    ...
    if (inIdx == CONTROL_SERIAL_USART2) {
        ...
        for (uint8_t i = 0; i < (IBUS_NUM_CHANNELS * 2); i += 2) {
            ibusL_captured_value[(i / 2)] = CLAMP(commandL.channels[i] + (commandL.channels[i + 1] << 8) - 1000, 0,
                                                  INPUT_MAX); // 1000-2000 -> 0-1000
        }
...
#ifdef CONTROL_IBUS
        for (uint8_t i = 0; i < (IBUS_NUM_CHANNELS * 2); i += 2) {
            ibusR_captured_value[(i / 2)] = CLAMP(commandR.channels[i] + (commandR.channels[i + 1] << 8) - 1000, 0,
                                                  INPUT_MAX); // 1000-2000 -> 0-1000
        }
...
void handleTimeout(void)
{
    ...
    if (inIdx == CONTROL_ADC) {
        ...
        if (IN_RANGE(input1[inIdx].raw, input1[inIdx].min - ADC_PROTECT_THRESH,
                     input1[inIdx].max + ADC_PROTECT_THRESH) &&
            IN_RANGE(input2[inIdx].raw, input2[inIdx].min - ADC_PROTECT_THRESH,
                     input2[inIdx].max + ADC_PROTECT_THRESH)) {
...
#if defined(DUAL_INPUTS) && ((defined(CONTROL_SERIAL_USART2) && CONTROL_SERIAL_USART2 == 1) ||                         \
                             (defined(SIDEBOARD_SERIAL_USART2) && SIDEBOARD_SERIAL_USART2 == 1))
    ...
    (defined(SIDEBOARD_SERIAL_USART2) && SIDEBOARD_SERIAL_USART2 == 0 && !defined(VARIANT_HOVERBOARD))
...
#if defined(DUAL_INPUTS) && ((defined(CONTROL_SERIAL_USART3) && CONTROL_SERIAL_USART3 == 1) ||                         \
                             (defined(SIDEBOARD_SERIAL_USART3) && SIDEBOARD_SERIAL_USART3 == 1))
        inIdx = 0; // Switch to Primary input in case of Timeout on Auxiliary input
#endif
    }
    else { // No Timeout
...
#if (defined(CONTROL_SERIAL_USART3) && CONTROL_SERIAL_USART3 == 0) ||                                                  \
    (defined(SIDEBOARD_SERIAL_USART3) && SIDEBOARD_SERIAL_USART3 == 0 && !defined(VARIANT_HOVERBOARD))
...
#endif
#if defined(DUAL_INPUTS) &&                                                                                            \
    ((defined(CONTROL_PPM_LEFT) && CONTROL_PPM_LEFT == 1) || (defined(CONTROL_PPM_RIGHT) && CONTROL_PPM_RIGHT == 1) || \
...
#if defined(CRUISE_CONTROL_SUPPORT) &&                                                                                 \
    (defined(SUPPORT_BUTTONS) || defined(SUPPORT_BUTTONS_LEFT) || defined(SUPPORT_BUTTONS_RIGHT))
    cruiseControl(button1); // Cruise control activation/deactivation
#endif
}

/*
 * Check for new data received on USART2 with DMA: refactored function from
...
void usart2_rx_check(void)
{
    ...
    if (pos != old_pos) {               // Check change in received data
        ...
        if (pos > old_pos &&
            (pos - old_pos) == commandL_len) { // "Linear" buffer mode: check if current position is over previous one
                                               // AND data length equals expected length
            memcpy(ptr, &rx_buffer_L[old_pos],
                   commandL_len); // Copy data. This is possible only if command_raw is contiguous! (meaning all the
                                  // structure members have the same size)
            usart_process_command(&commandL_raw, &commandL, 2); // Process data
        }
        else if ((rx_buffer_L_len - old_pos + pos) ==
                 commandL_len) { // "Overflow" buffer mode: check if data length equals expected length
    ...
    if (pos != old_pos) {                  // Check change in received data
        ...
        if (pos > old_pos &&
            (pos - old_pos) == Sideboard_L_len) { // "Linear" buffer mode: check if current position is over previous
                                                  // one AND data length equals expected length
            memcpy(ptr, &rx_buffer_L[old_pos],
                   Sideboard_L_len); // Copy data. This is possible only if Sideboard_raw is contiguous! (meaning all
                                     // the structure members have the same size)
            usart_process_sideboard(&Sideboard_L_raw, &Sideboard_L, 2); // Process data
        }
        else if ((rx_buffer_L_len - old_pos + pos) ==
                 Sideboard_L_len) { // "Overflow" buffer mode: check if data length equals expected length
            memcpy(ptr, &rx_buffer_L[old_pos], rx_buffer_L_len - old_pos); // First copy data from the end of buffer
            if (pos > 0) {                         // Check and continue with beginning of buffer
...
/*
 * Check for new data received on USART3 with DMA: refactored function from
 * https://github.com/MaJerle/stm32-usart-uart-dma-rx-tx
 * - this function is called for every USART IDLE line detection, in the USART interrupt handler
 */
void usart3_rx_check(void)
{
    ...
    if (pos != old_pos) {               // Check change in received data
        ...
        if (pos > old_pos &&
            (pos - old_pos) == commandR_len) { // "Linear" buffer mode: check if current position is over previous one
                                               // AND data length equals expected length
            memcpy(ptr, &rx_buffer_R[old_pos],
                   commandR_len); // Copy data. This is possible only if command_raw is contiguous! (meaning all the
                                  // structure members have the same size)
            usart_process_command(&commandR_raw, &commandR, 3); // Process data
        }
        else if ((rx_buffer_R_len - old_pos + pos) ==
                 commandR_len) { // "Overflow" buffer mode: check if data length equals expected length
    ...
    if (pos != old_pos) {                  // Check change in received data
        ...
        if (pos > old_pos &&
            (pos - old_pos) == Sideboard_R_len) { // "Linear" buffer mode: check if current position is over previous
                                                  // one AND data length equals expected length
            memcpy(ptr, &rx_buffer_R[old_pos],
                   Sideboard_R_len); // Copy data. This is possible only if Sideboard_raw is contiguous! (meaning all
                                     // the structure members have the same size)
            usart_process_sideboard(&Sideboard_R_raw, &Sideboard_R, 3); // Process data
        }
        else if ((rx_buffer_R_len - old_pos + pos) ==
                 Sideboard_R_len) { // "Overflow" buffer mode: check if data length equals expected length
            memcpy(ptr, &rx_buffer_R[old_pos], rx_buffer_R_len - old_pos); // First copy data from the end of buffer
            if (pos > 0) {                         // Check and continue with beginning of buffer
...
void usart_process_debug(uint8_t *userCommand, uint32_t len)
{
    ...
    static size_t pos                          = 0;
    static enum { WAIT_START, IN_FRAME } state = WAIT_START;
...
void usart_process_sideboard(SerialSideboard *Sideboard_in, SerialSideboard *Sideboard_out, uint8_t usart_idx)
{
    ...
    if (Sideboard_in->start == SERIAL_START_FRAME) {
        checksum = (uint16_t)(Sideboard_in->start ^ Sideboard_in->pitch ^ Sideboard_in->dPitch ^ Sideboard_in->cmd1 ^
                              Sideboard_in->cmd2 ^ Sideboard_in->sensors);
...
void sideboardSensors(uint8_t sensors)
{
    ...
    if (inIdx == sideboardIdx) { // Use Sideboard data
        sensor2_index =
            (sideboardSns & SWD_SET) >> 13; // SWD on RC transmitter is used to Activate/Deactivate Field Weakening
