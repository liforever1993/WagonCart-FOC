...
uint8_t        motorEnable  = 0;        // initially motors are disabled for SAFETY
...
void DMA1_Channel1_IRQHandler(void) {
  ...
  if(ABS(curL_DC) > curDC_max || motorEnable == 0) {
    LEFT_TIM->BDTR &= ~TIM_BDTR_MOE;
  } else {
    LEFT_TIM->BDTR |= TIM_BDTR_MOE;
  }

  if(ABS(curR_DC)  > curDC_max || motorEnable == 0) {
  ...
  enableFin = motorEnable && !rtY_Left.z_errCode && !rtY_Right.z_errCode;
