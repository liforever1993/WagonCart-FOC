...
extern uint8_t timeoutFlgADC; // Timeout Flag for for ADC Protection: 0 = OK, 1 = Problem detected (line disconnected or
                              // wrong ADC data)
extern uint8_t timeoutFlgSerial; // Timeout Flag for Rx Serial command: 0 = OK, 1 = Problem detected (line disconnected
                                 // or wrong Rx data)
...
int main(void)
{
    ...
    int32_t board_temp_adcFixdt =
        adc_buffer.temp << 16; // Fixed-point filter output initialized with current ADC converted to fixed-point
    ...
    printf_log("Drive mode %i selected: max_speed:%i acc_rate:%i \r\n", drive_mode, max_speed, rate);
#endif
    ...
    while (1) {
        if (buzzerTimer - buzzerTimer_prev > 16 * DELAY_IN_MAIN_LOOP) { // 1 ms = 16 ticks buzzerTimer
            ...
            speedBlend = (uint16_t)(((CLAMP(speedAvgAbs, 10, 60) - 10) << 15) /
                                    50); // speedBlend [0,1] is within [10 rpm, 60rpm]
#endif

#ifdef STANDSTILL_HOLD_ENABLE
            standstillHold(); // Apply Standstill Hold functionality. Only available and makes sense for VOLTAGE or
                              // TORQUE Mode
#endif

#ifdef VARIANT_HOVERCAR
            if (inIdx == CONTROL_ADC) { // Only use use implementation below if pedals are in use (ADC input)
                if (speedAvgAbs < 60) { // Check if Hovercar is physically close to standstill to enable Double tap
                                        // detection on Brake pedal for Reverse functionality
                    multipleTapDet(input1[inIdx].cmd, HAL_GetTick(),
                                   &MultipleTapBrake); // Brake pedal in this case is "input1" variable
                }

                if (input1[inIdx].cmd > 30) { // If Brake pedal (input1) is pressed, bring to 0 also the Throttle pedal
                                              // (input2) to avoid "Double pedal" driving
                    input2[inIdx].cmd = (int16_t)((input2[inIdx].cmd * speedBlend) >> 15);
                    cruiseControl((uint8_t)rtP_Left
                                      .b_cruiseCtrlEna); // Cruise control deactivated by Brake pedal if it was active
                }
            }
#endif

#ifdef ELECTRIC_BRAKE_ENABLE
            // Apply Electric Brake. Only available and makes sense for TORQUE Mode
            electricBrake(speedBlend,MultipleTapBrake.b_multipleTap);
#endif

#ifdef VARIANT_HOVERCAR
            if (inIdx == CONTROL_ADC) { // Only use use implementation below if pedals are in use (ADC input)
                if (speedAvg > 0) { // Make sure the Brake pedal is opposite to the direction of motion AND it goes to 0
                                    // as we reach standstill (to avoid Reverse driving by Brake pedal)
            ...
            if (input2[inIdx].cmd < 0) { // When Throttle is negative, it acts as brake. This condition is to make sure
                                         // it goes to 0 as we reach standstill (to avoid Reverse driving)
...
// ####### SET OUTPUTS (if the target change is less than +/- 100) #######
            ...
            if (nunchuk_connected == 0) {
                cmdL =
                    cmdL * 0.8f +
                    (CLAMP(distanceErr + (steering * ((float)MAX(ABS(distanceErr), 50)) * ROT_P), -850, 850) * -0.2f);
                cmdR =
                    cmdR * 0.8f +
                    (CLAMP(distanceErr - (steering * ((float)MAX(ABS(distanceErr), 50)) * ROT_P), -850, 850) * -0.2f);
                if (distanceErr > 0) {
            ...
            if ((distance / 1345.0) - setDistance > 0.5 &&
                (lastDistance / 1345.0) - setDistance > 0.5) { // Error, robot too far away!
                motorEnable = 0;
                beepLong(5);
                poweroff();
            }
            ...
            if (transpotter_counter % 500 == 0) {
                if (nunchuk_connected == 0 && motorEnable == 0) {
...
#define OUTPUT_FOR_VOFA 1
        if (main_loop_counter % 25 == 0) { // Send data periodically every 125 ms
            ...
            process_debug();
                ...
                printf_log("motor:%i,%i,%i,%i,%i,%i,%u,%u,%i,%i,%i,%i\r\n",
#else
                printf_log(
                    "in1:%i in2:%i cmdL:%i cmdR:%i BatV:%i Temp:%i hallL:%u hallR:%u iLA:%i iLB:%i iRB:%i iRC:%i \r\n",
#endif
                ...
                uint8_T hallL = (uint8_t)((!(LEFT_HALL_U_PORT->IDR & LEFT_HALL_U_PIN) << 2) |
                                          ...
                                          (!(LEFT_HALL_W_PORT->IDR & LEFT_HALL_W_PIN)));
#if defined(OUTPUT_FOR_VOFA)
                printf_log("motor:%i,%i,%i,%i,%i,%u,%i,%i\r\n",
#else
                printf_log("in1:%i in2:%i cmdL:%i BatV:%i Temp:%i hallL:%u iLA:%i iLB:%i \r\n",
#endif
                       input1[inIdx].raw, // 1: INPUT1
                       input2[inIdx].raw, // 2: INPUT2
                       cmdL,              // 3: output command: [-1000, 1000]
                ...
                printf_log("motor:%i,%i,%i,%i,%i,%u,%i,%i\r\n",
#else
                printf_log("in1:%i in2:%i cmdR:%i BatV:%i Temp:%i hallR:%u iRB:%i iRC:%i \r\n",
#endif
                       input1[inIdx].raw, // 1: INPUT1
                       input2[inIdx].raw, // 2: INPUT2
...
// ####### FEEDBACK SERIAL OUT #######
        ...
        if (main_loop_counter % 2 == 0) { // Send data periodically every 10 ms
            ...
            if (__HAL_DMA_GET_COUNTER(huart2.hdmatx) == 0) {
                ...
                Feedback.checksum =
                    (uint16_t)(Feedback.start ^ Feedback.cmd1 ^ Feedback.cmd2 ^ Feedback.speedR_meas ^
        ...
        if (TEMP_POWEROFF_ENABLE && board_temp_deg_c >= TEMP_POWEROFF &&
            ...
            printf_log("Powering off, temperature is too high\r\n");
            poweroff();
        }
        else if (BAT_DEAD_ENABLE && batVoltage < BAT_DEAD && speedAvgAbs < 20) {
            printf_log("Powering off, battery voltage is too low\r\n");
            poweroff();
        }
            ...
            beepCount(4, 24, 1);
                 ...
                 board_temp_deg_c >= TEMP_WARNING) { // 5 beeps (low pitch): Mainboard temperature warning
            beepCount(5, 24, 1);
        }
            ...
            beepCount(0, 10, 30);
                 ...
                 (((cmdR < -50 && cmdL < -50) && speedAvg < 0) ||
                  MultipleTapBrake.b_multipleTap)) { // 1 beep fast (high pitch): Backward spinning motors
            beepCount(0, 5, 1);
            backwardDrive = 1;
        }
        else { // do not beep
        ...
        if (inactivity_timeout_counter >
            ...
            printf_log("Powering off, wheels were inactive for too long\r\n");
            poweroff();
        }

        // HAL_GPIO_TogglePin(LED_PORT, LED_PIN);                 // This is to measure the main() loop duration with an
        // oscilloscope connected to LED_PIN Update states
        inIdx_prev       = inIdx;
